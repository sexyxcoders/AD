...
class AdBot:
    ...
    async def create_client(self, user_id: int, session_string: str = None):
        client = TelegramClient(StringSession(session_string or f'session_{user_id}'), API_ID, API_HASH)
...
async def policy_accepted(query):
    """User accepted policy → Phone prompt"""
...
async def verify_otp(update: Update, otp: str):
    """Verify OTP → Check 2FA → Dashboard"""
    user_id = update.effective_user.id
...
async def dashboard_message(update, user_id):
    ...
    await update.message.reply_text(
        "🎛️ **DASHBOARD - READY TO ADVERTISE!** 🎛️\n\n"
        "✅ **Logged in successfully**\n"
        "📱 **Flow:** Load Groups → Set Ad → Delay → START\n\n"
        "**👇 Use buttons below:**",
...
async def show_status(query, user_id):
    ...
    status = f"📊 **STATUS**\n\nGroups: **{groups}**\nDelay: **{user_data[user_id].get('delay', 0)}s**\nBot: {'🟢 ON' if running else '🔴 OFF'}"
